...
// ============================================================
// Pila de capas EXPLOTADAS (Escala natural / cultural / tecnologica),
// cada una una mini-escena 3D VIVA y animada, en la misma proyeccion
// axonometrica que la base, mostrando solo el dato relevante de esa
// escala (igual concepto que el diagrama de referencia por capas, pero
// con contenido real y animado en vez de imagenes estaticas):
//   - Natural: cuerpos de agua reales, con el Humedal El Burro creciendo
//     y encogiendo segun el mismo ciclo climatico anual real (precipitacion
//     mensual de Bogota) que ya se calculo para el modulo 3D completo.
//   - Cultural: sintaxis espacial de la red vial — cada tramo coloreado
//     segun su INTEGRACION real (centralidad de intermediacion
//     aproximada sobre el grafo de la red, azul=segregado, rojo=muy
//     integrado/con mas "atajos" de camino mas corto pasando por ahi).
//   - Tecnologica: la red vial con los vehiculos reales de la simulacion
...
(function () {
  ...
  function pointMiniCamera(mini, target, distance, azimuthDeg) {
    ...
    const x = target.x + Math.cos(az) * Math.cos(ELEV) * distance;
    const y = target.y + Math.sin(ELEV) * distance;
    const z = target.z + Math.sin(az) * Math.cos(ELEV) * distance;
    mini.camera.position.set(x, y, z);
    mini.camera.lookAt(target.x, target.y, target.z);
    mini.camera.updateProjectionMatrix();
  }

  fetch("./assets/kennedy_net.json")
    .then(r => r.json())
    .then(net => {
      ...
      // ---- Escala NATURAL: cuerpos de agua reales + Humedal El Burro
      // creciendo/encogiendo con el ciclo climatico anual real ----
      const miniNat = setupMini("canvasNatural");
      ...
      fetch("./assets/kennedy_water_bodies.json").then(r => r.json()).then(bodies => {
        const positions = [], uvs = [];
        bodies.forEach(bwater => {
          if (bwater.nombre === "Humedal El Burro") {
            natElBurroPts = bwater.pts;
            natElBurroCentro = { x: bwater.pts.reduce((s, p) => s + p[0], 0) / bwater.pts.length, y: bwater.pts.reduce((s, p) => s + p[1], 0) / bwater.pts.length };
            return;
          }
          const pts = bwater.pts.map(p => toScene(p[0], p[1]));
          if (pts.length < 3) return;
          const pts2d = pts.map(p => new THREE.Vector2(p.x, p.z));
          let tris; try { tris = THREE.ShapeUtils.triangulateShape(pts2d, []); } catch (e) { tris = []; }
          tris.forEach(([a, b, c]) => [a, b, c].forEach(idx => { positions.push(pts[idx].x, 0, pts[idx].z); uvs.push(0, 0); }));
        });
        const geo = new THREE.BufferGeometry();
        geo.setAttribute("position", new THREE.Float32BufferAttribute(positions, 3));
        const mat = new THREE.MeshBasicMaterial({ color: 0x6f95a8, side: THREE.DoubleSide, transparent: true, opacity: 0.85 });
        natWaterMat = mat;
        miniNat.sceneRoot.add(new THREE.Mesh(geo, mat));
        // Un contorno tenue del terreno (todas las manzanas), para dar contexto sin ser el foco
        rebuildNatElBurro(41.7);
      });
      const HUMEDAL_CICLO_MINI = [41.7, 42.6, 46.7, 50.0, 47.4, 41.5, 37.7, 34.1, 33.0, 38.6, 43.8, 43.3];
      function rebuildNatElBurro(expansionPct) {
        if (!natElBurroPts) return;
        if (natElBurroMesh) { miniNat.sceneRoot.remove(natElBurroMesh); natElBurroMesh.geometry.dispose(); }
        const scale = 1 + expansionPct / 100 * 0.6;
        const pts = natElBurroPts.map(p => {
          const ex = natElBurroCentro.x + (p[0] - natElBurroCentro.x) * scale;
          const ey = natElBurroCentro.y + (p[1] - natElBurroCentro.y) * scale;
        ...
        const geo = new THREE.BufferGeometry();
        geo.setAttribute("position", new THREE.Float32BufferAttribute(positions, 3));
        const mesh = new THREE.Mesh(geo, natWaterMat);
        miniNat.sceneRoot.add(mesh);
        natElBurroMesh = mesh;
      }
      let natMesIdx = 0, natLastStep = 0;
      function animateNatural(now) {
        requestAnimationFrame(animateNatural);
        if (now - natLastStep > 1400) {
          natLastStep = now;
      ...
      // ---- Escala CULTURAL: sintaxis espacial (integracion real de la
      // red vial, azul=segregado a rojo=muy integrado) ----
      const miniCul = setupMini("canvasCultural");
      miniCul.resizeMini(viewSize);
      pointMiniCamera(miniCul, { x: 0, y: 0, z: 0 }, camDist, 40);
      window.addEventListener("resize", () => miniCul.resizeMini(viewSize));
      fetch("./assets/kennedy_integracion.json").then(r => r.json()).then(edges => {
        const positions = [], colors = [];
        const cLow = new THREE.Color(0x3b6fb0), cHigh = new THREE.Color(0xe0463f);
        edges.forEach(e => {
          const pts = e.pts.map(p => toScene(p[0], p[1]));
          const t = Math.pow(e.integ, 0.35); // realce visual (la mayoria de las vias tiene integracion muy baja)
          const c = cLow.clone().lerp(cHigh, t);
          for (let i = 0; i < pts.length - 1; i++) {
            positions.push(pts[i].x, 0, pts[i].z, pts[i + 1].x, 0, pts[i + 1].z);
        ...
        miniCul.sceneRoot.add(new THREE.LineSegments(geo, mat));
      });
      ...
      // ---- Escala TECNOLOGICA: red vial + vehiculos reales de SUMO
      // moviendose en vivo ----
      const miniTec = setupMini("canvasTecnologica");
      miniTec.resizeMini(viewSize);
      pointMiniCamera(miniTec, { x: 0, y: 0, z: 0 }, camDist, 40);
      window.addEventListener("resize", () => miniTec.resizeMini(viewSize));
      const roadPositions = [];
      net.edges.forEach(([kind, pts]) => {
        const scenePts = pts.map(p => toScene(p[0], p[1]));
        for (let i = 0; i < scenePts.length - 1; i++) {
          roadPositions.push(scenePts[i].x, 0, scenePts[i].z, scenePts[i + 1].x, 0, scenePts[i + 1].z);
        }
      });
      const roadGeo = new THREE.BufferGeometry();
      roadGeo.setAttribute("position", new THREE.Float32BufferAttribute(roadPositions, 3));
      const roadMatMini = new THREE.LineBasicMaterial({ color: 0x8a929c, transparent: true, opacity: 0.7 });
      miniTec.sceneRoot.add(new THREE.LineSegments(roadGeo, roadMatMini));

      const VEH_POOL_MINI = 1200;
      const vehGeoMini = new THREE.BoxGeometry(0.9, 0.5, 1.8);
      const vehMatMini = new THREE.MeshBasicMaterial({ color: 0xe2635a });
      const vehMini = new THREE.InstancedMesh(vehGeoMini, vehMatMini, VEH_POOL_MINI);
      miniTec.sceneRoot.add(vehMini);
      const dummyMini = new THREE.Object3D();
      let timestepsMini = [];
      fetch("./assets/kennedy_vehiculos.json").then(r => r.json()).then(data => {
        timestepsMini = data.map(([time, vs]) => ({ time, vehicles: vs.map(([id, x, y]) => ({ id, x, y })) }));
      });
      let miniTime = 0, miniLastFrame = 0;
      function animateTecnologica(now) {
        requestAnimationFrame(animateTecnologica);
        if (timestepsMini.length) {
          const dt = miniLastFrame ? Math.min(0.05, (now - miniLastFrame) / 1000) : 0;
          miniLastFrame = now;
          miniTime += dt * 12;
          const maxT = timestepsMini[timestepsMini.length - 1].time;
          if (miniTime > maxT) miniTime = 0;
          let idx = Math.floor((miniTime / maxT) * (timestepsMini.length - 1));
          idx = Math.max(0, Math.min(timestepsMini.length - 1, idx));
          const vehicles = timestepsMini[idx].vehicles;
          const n = Math.min(vehicles.length, VEH_POOL_MINI);
          for (let i = 0; i < n; i++) {
            const p = toScene(vehicles[i].x, vehicles[i].y);
            dummyMini.position.set(p.x, 0.05, p.z);
            dummyMini.updateMatrix();
            vehMini.setMatrixAt(i, dummyMini.matrix);
          }
          vehMini.count = n;
